...
const PrimaryTasks = ({ primaryTasks }) => <ContentRow label="Primære opgaver" text={primaryTasks} />;

const Description = ({ description }) => <ContentRow label="" text={description} />;

const Tools = ({ techStack }) => <ContentRow label="Sprog, tools og libraries" text={techStack} />;

const Work = ({ work }) => {
  const { address, name } = work.company;
  const { title, startDate, endDate, primaryTasks, description, techStack } = work.position;
